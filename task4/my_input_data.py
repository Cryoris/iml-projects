...
def extract_labels(filename, train_dir, one_hot=False):
  ...
  print('Extracting', filename)
  with open(train_dir+filename) as file:
    labels = numpy.loadtxt(file, delimiter=',').astype(int)
    if one_hot:
...
class SemiDataSet(object):
    def __init__(self, l_images, l_labels, unl_images, unl_labels, n_labeled):
        self.n_labeled = n_labeled
        self._images = numpy.append(unl_images,l_images,axis=0)

        # Unlabled DataSet
        self.unlabeled_ds = DataSet(unl_images, unl_labels)
...
def read_data_sets(train_dir, n_labeled = 100, fake_data=False, one_hot=False):
  ...
  TRAIN_L_IMAGES = "train_labeled_withoutLabel_useTrain.csv" 
  TRAIN_L_LABELS = "train_labeled_justLabel_useTrain.csv"  
  TRAIN_UNL_IMAGES = "train_unlabeled.csv" 
  TRAIN_UNL_LABELS = "train_unlabeled_dummyLabels.csv"
  TEST_IMAGES = "train_labeled_withoutLabel_useTest.csv"
  TEST_LABELS = "train_labeled_justLabel_useTest.csv"
  EVAL_IMAGES = "test.csv"
  VALIDATION_SIZE = 0

  train_l_images = extract_features(TRAIN_L_IMAGES, train_dir)
  train_l_labels = extract_labels(TRAIN_L_LABELS, train_dir, one_hot=one_hot)
  train_unl_images = extract_features(TRAIN_UNL_IMAGES, train_dir)
  train_unl_labels = extract_labels(TRAIN_UNL_LABELS, train_dir, one_hot=one_hot)
  ...
  data_sets.train = SemiDataSet(train_l_images, train_l_labels, train_unl_images, train_unl_labels, n_labeled)
